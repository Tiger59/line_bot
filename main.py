...
def create_message(input):
    # 3. 条件分岐あり
    place2id = {"稚内": "011000", "旭川": "012010", "留萌": "012020", "網走": "013010", "北見": "013020", "紋別": "013030",
    ...
    f = open('inputs.txt', 'w')  # 書き込みモードで開く
    f.write(input)  # 引数の文字列をファイルに書き込む
    f.close()  # ファイルを閉じる
    url = 'http://weather.livedoor.com/forecast/webservice/json/v1'
    if input not  in place2id.keys():
        message = TextSendMessage('対応していません')
    else:

        params = {'city': place2id[input] }
        response = requests.get(url, params=params)
        message = TextSendMessage( response.json()['description']['text'])
